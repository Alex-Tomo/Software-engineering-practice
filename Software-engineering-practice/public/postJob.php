...
<div id='postJobContainer'>
    <h1>Post a Job</h1>
    <form id='postJobForm' style='position: relative;' enctype='multipart/form-data' method='post' action='handlers/post_job_handler.php'>
...
for($job_index = 0; $job_index < sizeof($job_codes); $job_index++) {
    $page->addPageBodyItem("<option value='{$job_names[$job_index]}' id='{$job_names[$job_index]}' name='{$job_codes[$job_index]}'>");
        ...
        $page->addPageBodyItem("<button id='postJobBtn' class='clickable' type='submit'>Post Job</button> 
    </form>
</div>");
